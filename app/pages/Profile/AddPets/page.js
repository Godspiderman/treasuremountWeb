...
function AddPet() {
  ...
  const handleEditClick = (product) => {
    router.push(`/pages/Profile/PetAdd?id=${product.id}`);
  };
    ...
    <div className='AddPet-page'>
      <div className='AddPet-container box-shadow'>
        <div className='AddPet-head p-15'>
          <h2>Pets</h2>
          <div className="btns">
            <span onClick={() => router.push(`/pages/Profile/PetAdd`)} style={{ cursor: 'pointer' }}>
              <FaPlus /> Add Pets
            </span>
          </div>
        </div>
                  ...
                  paginatedProducts.map((product, index) => (
                    <tr key={index}>
                      ...
                      <td className='text-center'>
                        <p className={
                          product.productStatusId === 2 ? 'status-approved' :
